...
class bbRpc_setUrl extends apiModule
{
    public function setUrl($rpcUrl = false)
    {
        ...
        else if ($this->rpcUrl !== false)
        {
            ...
            elseif (RPF_API_MODULE_BBRPC_AUTH_DF_ORDER !== false){
                $dfOrder = explode("_",RPF_API_MODULE_BBRPC_AUTH_DF_ORDER);
                $this->setUrl(sprintf(RPF_API_MODULE_BBRPC_SETURL_PATTERN, $dfOrder[0]));
                log::debug("Fetching rpcUrl from config.php individual df order: {$this->rpcUrl}", __METHOD__);
                return true;
            }
            else {
